...
abundance_BR = pd.read_table('results/output/seasonalAbundances_B.csv', sep=",")
abundance_NB = pd.read_table('results/output/seasonalAbundances_W.csv', sep=",")

# Load matrix of pairwise distance between every hexagons on the grid
distanceMatrix = np.loadtxt('results/output/climate_distanceMatrix.csv', delimiter=",")
...
for s in abundance_BR.columns:
  ...
  np.savetxt("results/output/ORSIM_climate_results_" + s + ".csv", EMD_results, delimiter=',') # Save simulated migratory connectivity
