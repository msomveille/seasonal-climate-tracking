...
abundance_BR = pd.read_table('results/seasonalAbundances_B.csv', sep=",")
abundance_NB = pd.read_table('results/seasonalAbundances_W.csv', sep=",")

# Load matrix of pairwise distance between every hexagons on the grid
distanceMatrix = np.loadtxt('results/distanceMatrix.csv', delimiter=",", skiprows=1)
...
for s in abundance_BR.columns:
  ...
  np.savetxt("results/ORSIM_results_" + s + ".csv", EMD_results, delimiter=',') # Save simulated migratory connectivity
